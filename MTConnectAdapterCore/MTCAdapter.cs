...
namespace MTConnect {
  ...
  public class Adapter {
    ...
    public void SendChanged(String timestamp = null) {
      if (Verbose){
        Console.WriteLine("Sending Changed DataItems");
      }
      if (mBegun)
      ...
      foreach (DataItem di in this.mDataItems) {
    ...
    public void SendAllTo(Stream aClient) {
      lock (aClient) {
        ...
        byte[] message = this.mEncoder.GetBytes(line.ToCharArray());
        aClient.Write(message, 0, message.Length);

        foreach (DataItem di in separate) {
          line = timestamp;
          line += "|" + di.ToString() + "\n";
          message = this.mEncoder.GetBytes(line.ToCharArray());
          this.WriteToClient(aClient, message);
        }
    ...
    public void SendToAll(string line) {
      ...
      if (Verbose){
        System.Diagnostics.Debug.WriteLine("[MTCAdapter.SendToAll] Sending: " + line, "MTCAdapter");
    ...
    private void WriteToClient(Stream aClient, byte[] aMessage) {
      try {
        ...
        System.Diagnostics.Debug.WriteLine("[MTCAdapter.WriteToClient] Error: " + e.ToString(), "MTCAdapter");
        Console.WriteLine("Error during write: " + e.Message);
        try { aClient.Close(); } catch (Exception f) {
    ...
    private void ListenForClients() {
      ...
      try {
        while (this.mRunning) {
          ...
          TcpClient client = this.mListener.AcceptTcpClient();
          //create a thread to handle communication 
          //with connected client
          Thread clientThread = new Thread(new ParameterizedThreadStart(this.HeartbeatClient));
          clientThread.Start(client);
          this.SendAllTo(client.GetStream());
          clientThread.Join();
        }
      } catch (Exception e) {
        System.Diagnostics.Debug.WriteLine("[MTCAdapter.ListenForClients] Error: ", e.ToString(), "MTCAdapter");
    ...
    public void Stop() {
      if (this.mRunning) {
        ...
        this.mListenThread.Join(2 * this.Heartbeat);

        foreach (Object obj in this.mClients) {
